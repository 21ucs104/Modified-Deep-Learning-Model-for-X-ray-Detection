...
def extract_data(dir_path):
  ...
  if dir_path == '/kaggle/input/chest-xray-pneumonia/chest_xray/train':
      for c in classes:
        if c =='PNEUMONIA':
...
def identity_block(X, f, filters, stage, block):
  ...
  # First component/layer of main path
  Y = Conv2D(filters= f1, kernel_size = (1,1), strides = (1,1), padding='valid', name=conv_base_name+'first_component', kernel_initializer = glorot_uniform(seed=0))(Y)
  Y = BatchNormalization(axis=bn_axis, name=bn_base_name+'first_component')(Y)
  Y = Activation('relu')(Y)

  Z = Conv2D(filters=f1, kernel_size = (1,1) , strides = (1,1), padding='valid', name = conv_base_name+'first_first_component' ,kernel_initializer = glorot_uniform(seed=0))(Z)
  Z = BatchNormalization(axis=bn_axis, name=bn_base_name+'first_first_component')(Z)
  Z = Activation('relu')(Z)
  ...
  # Second component/layer of main path
  Y = Conv2D(filters= f2, kernel_size = (f,f), strides = (1,1), padding='same', name=conv_base_name+'second_component', kernel_initializer = glorot_uniform(seed=0))(Y)
  Y = BatchNormalization(axis=bn_axis, name=bn_base_name+'second_component')(Y)
  Y = Activation('relu')(Y)
    
  # Second component/layer of main path
  Z = Conv2D(filters= f2, kernel_size = (f,f), strides = (1,1), padding='same', name=conv_base_name+'second_first_component', kernel_initializer = glorot_uniform(seed=0))(Z)
  Z = BatchNormalization(axis=bn_axis, name=bn_base_name+'second_first_component')(Z)
  Z = Activation('relu')(Z)
...
#   Y = X
  ...
  # Third component/layer of main path
  Y = Conv2D(filters= f3, kernel_size = (1,1), strides = (1,1), padding='valid', name=conv_base_name+'third_component', kernel_initializer = glorot_uniform(seed=0))(Y)
  Y = BatchNormalization(axis=bn_axis, name=bn_base_name+'third_component')(Y)

  # "Addition step" - skip-connection value merges with main path
  # NOTE: both values have same dimensions at this point, so no operation is required to match dimensions
  Y = Add()([Y, X_skip_connection])
  Y = Activation('relu')(Y)
